...
function create_callback(){
	...
	if(!$callback){
	 $my_post = array(
          ...
          'post_content'  => 'Sofern Sie sich angemeldet haben, werden Ihre Daten nun verarbeitet. \n Das Fenster schließt sich automatisch.',
          'post_status'   => 'publish',
          'post_author'   => '',
          'post_category' => '',
          'post_type'     => 'page'
...
function put_user_top_reads($body){
	...
	sApiCustom("https://ajec5zkbxf.execute-api.eu-west-1.amazonaws.com/ba_dev/increment-adds", "GET", $headers);
...
function show_ukv_tops(){
	...
	sApiCustom("https://ajec5zkbxf.execute-api.eu-west-1.amazonaws.com/ba_dev/increment-reads", "GET", $headers);
	...
	if($q != NULL){
		...
		$resArr[0] = array(
			"artistName" => "",
			"trackTitle" => "Noch keine Datenbankeinträge vorhanden.",
			"albumTitle" => "",
			"songUrl" => "",
			"albumImg" => ""
		);
